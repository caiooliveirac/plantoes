...
import { MealBreakUserError, restartCurrentMealBreakSession } from "@/modules/telegram/meal-breaks";
...
export async function POST(request: NextRequest) {
    ...
    try {
        const result = await restartCurrentMealBreakSession({
        ...
        await getDb().insert(auditLogs).values({
            actorUserId: session.user.id,
            action: "meal_break.session_restarted",
        ...
        const status = error instanceof MealBreakUserError ? 400 : 500;
        return NextResponse.json(
            { error: error instanceof Error ? error.message : "Não consegui reiniciar a divisão." },
            { status },
        );
    }
}
