 ...
 * que ninguém tivesse pedido isso. Em notes esses lançamentos carregam o
 * marcador do bypass — "sem gate de saldo" ou, na redação anterior,
 * "chefia 2031".
 *
 ...
 *
 * Caso órfão: quando o plantão verde já foi removido à mão do fechamento, sobra
 * só o débito. Aí não há o que converter — apagar o settlement é a correção
 * inteira (o plantão, se devido, já foi relançado pela coordenação).
 *
 * Uso:
 *   tsx scripts/convert-chief-bypass-extras.ts                    # só lista (dry-run)
 *   tsx scripts/convert-chief-bypass-extras.ts --doctor=CECILIA   # filtra por nome
...
/** Filtro por nome (substring, sem acento/caixa — casa com normalized_name). */
const doctorFilter = process.argv
    .find((arg) => arg.startsWith("--doctor="))
    ?.slice("--doctor=".length)
    .trim()
    .toUpperCase() ?? null;
...
async function main() {
    ...
    const rows = (await db.execute(sql`
        ...
        where (s.notes like '%sem gate de saldo%' or s.notes like '%chefia 2031%')
          ${doctorFilter ? sql`and d.normalized_name like ${`%${doctorFilter}%`}` : sql``}
    ...
    for (const row of rows) {
        console.log(
            ...
            + ` · saldo devolvido: ${-row.deltaMinutes} min`
            + `${row.adminExtraShiftId ? "" : " · ÓRFÃO (plantão verde já removido à mão)"}`
            + ` · ${row.notes}`,
        );
    }
