export const BANK_HOURS_RULE_VERSION = 1;
...
export function calculateBankHours(input: BankHoursCalculationInput): BankHoursCalculationResult {
    ...
    const overtimeMinutes = Math.max(0, diffMinutes(scheduledEndAt, actualEndAt));
    ...
    const explanation = arrivalDelayMinutes === 0
        ? (overtimeMinutes > 0
            ? "Chegou dentro da tolerancia de 15 min e recebeu excedente em dobro."
            : "Chegou dentro da tolerancia e nao gerou debito nem credito adicional.")
        : (overtimeMinutes > 0
            ? `Chegou com ${arrivalDelayMinutes} min de atraso e o excedente ficou simples.`
